...
MarkdownDeck bundles an `mddeck` Claude skill inside the app. It teaches Claude the slide markdown format (front-matter, `---` slide splits, the eight layout presets, placing content into named columns with `::: cell <name>` blocks, and custom `<!-- grid: … -->` columns), the content blocks (Mermaid/PlantUML, Vega-Lite charts, CSV/TSV tables, KaTeX math, admonitions), the `mddeck` CLI verbs, and the macOS editor's shortcuts. With it installed, Claude can draft and edit decks in the exact syntax MarkdownDeck renders — no guessing at conventions.
...
Claude will produce (or edit) a Markdown file in MarkdownDeck's syntax — front-matter at the top, `---` between slides, `<!-- layout: … -->` directives, `::: cell <name>` blocks (or a custom `<!-- grid: … -->`) to place content into named columns, and the right fenced blocks for diagrams, charts, tables, and math.
...
- [`./03-layouts.md`](./03-layouts.md) — the layout presets, named columns, `::: cell` placement, and custom `<!-- grid: … -->` the skill uses for multi-column slides.
